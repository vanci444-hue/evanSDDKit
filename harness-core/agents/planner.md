...
先接收主智能体已与用户确定的 `execution_mode`，按 [推进模式](../protocols/execution-mode.md)（路径：`<harness_root>/harness-core/protocols/execution-mode.md`）记录 `automatic` 或 `step_by_step`。已有计划沿用选择；新计划未选择时交主智能体说明并询问，不从模板示例推定。两种模式使用同一份任务依赖，不为逐步执行添加虚假依赖。
...
有前端任务的新计划必须按 [前端门禁](../protocols/frontend-gate.md) 在最后一个前端任务设置唯一 `user_gate`；该任务承担整体 Mock 收口，依赖其他前端任务，不能只靠 priority 声称它最后执行。前端全部技术验收后停止项目的新派发，由用户验收页面并填写后端 API 配置后再继续。自动模式在门禁前允许独立后端并行；逐步模式一次一个任务。联调、交付等后续阶段在两种模式下都须等待前端门禁；无前端则不设此前端门禁，仍遵守所选推进模式。
...
- 首次生成计划时，按任务数组顺序从 `T-001` 开始连续递增，包含最终交付任务；例如 14 个任务使用 `T-001`～`T-014`。不得按前端、后端、联调或交付分段编号、跳号或预留号段。
- 工作类型由 `type` 表达，执行先后由 `dependencies` 和 `priority` 决定；编号不代表任务必须串行执行。
- 已开始执行的计划保留现有 ID，新增任务从历史最大编号继续递增，不复用删除任务的编号，不为了消除历史空号而重编号。
- 用户明确要求整理尚未执行的计划时，可一次性连续重编号，同时更新依赖、门禁、描述、检查项及其他引用中的任务 ID。存在已派发或执行记录时，先协调编排器与运行中的智能体，不单独改 ID。
...
| `execution_mode` | 用户选择的 `automatic / step_by_step`；新计划必填，旧计划缺失兼容 automatic |
| `step_gate` | 顶层当前任务确认点，按推进模式协议初始化 kind=task_review、task_id=null、status=pending、user_confirmation=null；已有计划保留真实状态 |
...
| 字段 | 约定 |
|---|---|
| `id`、`title`、`type`、`description` | 按上述编号规则生成的唯一任务 ID、标题、工作类型及明确交付范围 |
...
状态集合为 `pending / in_progress / testing / fixing / passed / blocked`。任务里的 `user_gate` 只设置在前端整体收口处；逐步模式的任务确认由顶层 `step_gate` 复用，不给每项任务复制审批结构。页面功能 HTML 展示经源码核对的功能与实现，不另建进度数据源。上述 PRD 来源与 AC 约定对框架交付任务按上一节的明确例外处理。
...
先核对 JSON 可解析、ID 唯一、引用有效、规范与项目一致；首次生成的任务数组还须逐项满足 `tasks[i].id == "T-" + 三位补零(i + 1)`，不得仅检查唯一性而放过分段或跳号。已有执行历史的计划按上述编号规则保留 ID，不套用首次生成的连续性检查。再对照当前 PRD 检查本轮 AC 的完整覆盖与来源。编号出现不代表完整验收已被安排；无法识别来源格式时明确报告未能核对的部分，不记为通过。
...
核对 execution_mode 来自用户选择、step_gate 初始化完整。逐步模式只返回一个当前任务候选；推演当前任务 passed、用户尚未确认时候选应为空，确认后才允许下一任务。返回摘要中说明模式、用户停顿点和潜在并行候选；逐步模式不把潜在并行候选当成本轮实际派发清单。
